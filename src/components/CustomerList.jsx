...
import { formatCurrency, buildWhatsAppReminderUrl } from '../utils/formatters';
import { 
  Search, 
  UserPlus, 
  MessageCircle, 
...
export function CustomerList({ onSelectCustomer, onAddCustomer, onQuickAddTransaction }) {
  const { 
    customers, 
    businessName, 
    customerListFilter, 
  ...
  const filteredCustomers = customers.filter(c => {
    ...
    if (filterType === 'GET') return c.netBalance > 0;
          ...
          <h2 className="text-xl font-extrabold text-slate-900 dark:text-white flex items-center space-x-2">
            ...
            {filterType === 'SETTLED' && (
              <span className="text-xs px-2.5 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 font-extrabold border border-emerald-300 dark:border-emerald-800 flex items-center space-x-1">
                ...
                <span>Showing Settled Accounts</span>
              </span>
            )}
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Select a customer to view complete transaction history or add payments
          </p>
        </div>
      ...
      {filteredCustomers.length === 0 ? (
        <div className="py-12 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl">
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
            No customers found matching your filter options.
          </p>
          <button
            onClick={onAddCustomer}
          ...
          >
            ...
            <span>Add a new customer now</span>
          </button>
        </div>
      ) : (
